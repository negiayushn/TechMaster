/**
 * Client-side interview bank generator (100+ items per major technology).
 * Mirrors scripts/generate-interviews.mjs without Node fs imports.
 */
(function (global) {
  const TECH_CONFIG = {
  ...
  function jsSubtopics() {
    return semanticSubtopics("JavaScript", "event loop", "execution model").map(function (s, i) {
      return i < 12 ? s : { id: s.id + "-js", name: s.name + " (JS runtime)", hook: s.hook };
    });
  }
  ...
  function expressSubtopics() {
    return nodeSubtopics().slice(0, 22).map(function (s) {
      return {
        id: s.id,
        name: s.name.indexOf("Express") >= 0 ? s.name : "Express: " + s.name,
  ...
  function pythonSubtopics() {
    return dsSubtopics().map(function (s) {
      return { id: s.id, name: "Python: " + s.name, hook: s.hook + " with CPython ergonomics" };
    });
  }
  ...
  function answerParagraph(techLabel, sub, diff) {
    var hook = sub.hook || "real-world constraints";
    if (diff === "beginner")
      return (
        "Start with a crisp definition of " +
        sub.name +
        " in " +
        techLabel +
        ", then connect it to " +
        hook +
        ". Mention the primary user-visible outcome, the main moving parts teams configure, and one concrete checklist item you always verify in code review (correctness, safety, or operability). Close with how this topic interacts with adjacent concerns like performance, security, or maintainability."
      );
    if (diff === "intermediate")
      return (
        "Walk through a practical approach: requirements, constraints, and a stepwise implementation plan for " +
        sub.name +
        ". Discuss tradeoffs teams debate in production (latency vs complexity, consistency vs availability, DX vs strictness), and name monitoring or tests that prove the behavior. Tie it back to " +
        hook +
        " so the answer sounds like shipping experience, not textbook memorization."
      );
    if (diff === "advanced")
      return (
        "Go deep on failure modes: race conditions, scaling cliffs, misleading metrics, and 'works on my machine' traps for " +
        sub.name +
        ". Explain how you would prove correctness under concurrency, how you would bound blast radius during rollout, and what observability signals you would watch. Reference " +
        hook +
        " explicitly and give a senior-level mitigation pattern (automation, guardrails, or platform defaults)."
      );
    if (diff === "system")
      return (
        "Frame the discussion around bottlenecks and contracts: which service owns " +
        sub.name +
        ", how data flows across boundaries, and what SLIs degrade first under load. Compare at least two architectural options, including operational cost and team cognitive load. Anchor the narrative in " +
        hook +
        " and end with a phased rollout plan and rollback triggers."
      );
    return (
      "Describe a disciplined incident workflow: reproduce, narrow (binary search configs/versions), inspect logs/traces/metrics, form hypotheses, and run a minimal experiment. For " +
      sub.name +
      ", list two common misconfigurations and one subtle code bug pattern. Explain verification (canary, shadow traffic, feature flag) and how you would prevent recurrence via tests, guardrails, or runbooks tied to " +
      hook +
      "."
    );
  }
  ...
  function exampleSnippet(techKey, diff, subId) {
    var codeByTech = {
      javascript:
        "// Example: defensive async sequencing\nasync function fetchUserSafe(id) {\n  const res = await fetch(`/api/users/${id}`);\n  if (!res.ok) throw new Error(`HTTP ${res.status}`);\n  return res.json();\n}",
      react:
        "// Example: stable callback pattern sketch\nfunction useLiveRef(cb) {\n  const r = useRef(cb);\n  useEffect(() => { r.current = cb; }, [cb]);\n  return r;\n}",
      nodejs:
        "// Example: bounded concurrency pool\nasync function mapPool(items, limit, worker) {\n  const ret = new Array(items.length);\n  let i = 0;\n  const runners = new Array(Math.min(limit, items.length)).fill(0).map(async () => {\n    while (i < items.length) {\n      const idx = i++;\n      ret[idx] = await worker(items[idx], idx);\n    }\n  });\n  await Promise.all(runners);\n  return ret;\n}",
      docker:
    ...
    var base = codeByTech[techKey] || "// " + techKey + ": illustrate " + subId + " with a minimal, testable snippet tied to your last project.";
    return diff === "debugging" ? base + "\n\n// Add: structured logs + correlation id" : base;
  }

  function mistakesList(diff) {
    var common = [
    ...
    if (diff === "debugging") return ["Skipping reproduction steps.", "Changing multiple variables at once."].concat(common);
    return common;
  }
  ...
  function buildQuestion(techKey, techLabel, sub, variantKey, index) {
    var v = DIFF_VARIANTS.find(function (x) {
      return x.key === variantKey;
    });
    if (!v) v = DIFF_VARIANTS[0];
    return {
      id: techKey + "-" + sub.id + "-" + variantKey + "-" + index,
      technology: techKey,
      technologyLabel: techLabel,
      difficulty: v.key,
      category: classify(v.key),
      question: questionStem(techLabel, sub.name, v.key),
      answer: answerParagraph(techLabel, sub, v.key),
      strategy: answerStrategy(v.key),
      example: exampleSnippet(techKey, v.key, sub.id),
      mistakes: mistakesList(v.key),
  ...
  function buildInterviewBank() {
    var out = [];
    Object.keys(TECH_CONFIG).forEach(function (techKey) {
      var cfg = TECH_CONFIG[techKey];
      cfg.subtopics.forEach(function (sub) {
        DIFF_VARIANTS.forEach(function (variant) {
  ...
  global.TMInterviewGen = { buildInterviewBank: buildInterviewBank, BANK_VERSION: "v1" };
})(window);
